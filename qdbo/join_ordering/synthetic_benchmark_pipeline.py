...
# QDBO join-ordering pipeline over synthetic instances.
# Layout:
#   {SYNTHETIC_ROOT}/{graph_type}/{relations}/{instance_id}
# Filter:
#   ONLY_INSTANCE_ID filters by instance folder name under each relations folder, e.g. keep */18relations/0, */10relations/0, ...
...
from qdbo.join_ordering.backend.bqm_solver_execution import build_bqm, solve_bqm, read_out
from qdbo.join_ordering.backend.nl_solver_execution import nl_query_optimization
from qdbo.join_ordering.backend.cqm_solver_execution import build_cqm, solve_cqm
from qdbo.join_ordering.backend.jo_utils import process_input, parse_selectivities
from qdbo.join_ordering.backend.jo_utils import compute_db_cost
...
DEFAULT_BENCHMARK_ROOT = PROJECT_ROOT / "workloads" / "join_ordering"
BENCHMARK_ROOT = Path(os.environ.get("QDBO_BENCHMARK_ROOT", DEFAULT_BENCHMARK_ROOT))

SYNTHETIC_ROOT = str(BENCHMARK_ROOT / "synthetic")

RESULTS_ROOT = Path(os.environ.get("QDBO_RESULTS_ROOT", PROJECT_ROOT / "results"))
OUT_DIR = str(RESULTS_ROOT / "join_ordering/synthetic_cpp_overhead")
LOG_DIR = str(RESULTS_ROOT / "join_ordering/synthetic_cpp_overhead/txt")
CSV_PREFIX = "cpp_iter_synthetic_cost"
LOG_PREFIX = "cpp_iter_synthetic_cost"


RUN_NL = False
RUN_CQM = False
RUN_BQM = False
RUN_ITER = False  # iterative solver

# ---- NL/CQM configs ----
NL_TIME_LIMIT_S = 5
CQM_TIME_LIMIT_S = 5
NL_SAMPLER_LABEL = "QDBO NL Solver (Synthetic)"
CQM_LABEL = "QDBO CQM (Synthetic)"
...
ITER_LABEL = "IterativeSolver (Synthetic)"

ROUNDS = 1

# Filter by instance folder name under each relations folder
# None = run all instance_ids (0/1/2/3/...)
ONLY_INSTANCE_ID: Optional[int] = 0
...
def parse_synthetic_path(inst_dir: str, root: str) -> Dict[str, Optional[str]]:
    """
    Expected layout:
      {root}/{graph_type}/{relations}/{instance_id}
    Example:
      Synthetic/chain/18relations/0
    """
    try:
        rel = os.path.relpath(inst_dir, root)
        parts = rel.split(os.sep)

        graph_type = parts[0] if len(parts) >= 1 else None
        relations = parts[1] if len(parts) >= 2 else None
...
def find_instance_folders(base_dir: str, only_instance_id: Optional[int] = None) -> List[str]:
    """
    Recursively find folders that contain both cardinalities.json and selectivities.json.

    Filtering semantics:
    - only_instance_id filters the *instance folder name* under each relations folder:
        keep folders whose path matches */{relations}/{only_instance_id}
      Example: keep all .../18relations/0, .../10relations/0, ...

    If only_instance_id is None -> return all instances.
    """
    base = Path(base_dir)
    if not base.exists():
    ...
    for p in base.rglob("*"):
        if not p.is_dir():
            continue
        if (p / "cardinalities.json").is_file() and (p / "selectivities.json").is_file():
            inst_dirs.append(str(p))

    inst_dirs = sorted(inst_dirs, key=lambda x: _natural_key(os.path.relpath(x, base_dir)))

    if only_instance_id is None:
        return inst_dirs

    want = str(only_instance_id)
    filtered: List[str] = []
    for d in inst_dirs:
        meta = parse_synthetic_path(d, base_dir)
        if meta.get("instance_id") == want:
            filtered.append(d)
...
def run_blackbox_with_timeout(
    ...
    env["QDBO_CUSTOM_EMB"] = str(emb)
    env["QDBO_ITERATIONS"] = "" if iterations is None else str(iterations)
...
iters_raw = os.environ.get("QDBO_ITERATIONS", "")
...
# Core runner
# -----------------------
def run_synthetic_cost() -> Tuple[Optional[str], Optional[str], Optional[str], Optional[str], str]:
    _ensure_dir(OUT_DIR)
    _ensure_dir(LOG_DIR)

    run_ts = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")

    nl_csv_path = os.path.join(OUT_DIR, f"{CSV_PREFIX}_{run_ts}_nl.csv") if RUN_NL else None
    cqm_csv_path = os.path.join(OUT_DIR, f"{CSV_PREFIX}_{run_ts}_cqm.csv") if RUN_CQM else None
    bqm_csv_path = os.path.join(OUT_DIR, f"{CSV_PREFIX}_{run_ts}_bqm.csv") if RUN_BQM else None
    ...
    iter_csv_path = os.path.join(OUT_DIR, f"{iter_prefix}_{run_ts}_iter.csv") if RUN_ITER else None
    log_prefix = iter_prefix if RUN_ITER else CSV_PREFIX
    ...
    print(f"[FILTER] ONLY_INSTANCE_ID={ONLY_INSTANCE_ID} (None = all)")
    if nl_csv_path:
    ...
    folders = find_instance_folders(SYNTHETIC_ROOT, only_instance_id=ONLY_INSTANCE_ID)
    print(f"[INFO] total_instances={len(folders)} (after filter)")
    ...
    try:
        ...
        for r in range(ROUNDS):
            ...
            for i, inst_dir in enumerate(folders):
                ...
                meta = parse_synthetic_path(inst_dir, SYNTHETIC_ROOT)
                ...
                if need_process_input:
                    try:
                        ...
                        print(f"[ERR] [{i + 1}/{len(folders)}] {meta.get('relative_path')} {msg}")
                        traceback.print_exc()

                        # Write error rows for enabled solvers
                        if RUN_NL and nl_writer is not None:
                            row = _make_base_row(run_ts, meta, i, inst_dir, inst_name, "nl", r)
                        ...
                        if RUN_CQM and cqm_writer is not None:
                            row = _make_base_row(run_ts, meta, i, inst_dir, inst_name, "cqm", r)
                            row["notes"] = "process_input_failed"
                            row["error"] = str(e)
                            cqm_writer.writerow({k: row.get(k, None) for k in base_header})
                            cqm_fcsv.flush()
                        if RUN_BQM and bqm_writer is not None:
                            row = _make_base_row(run_ts, meta, i, inst_dir, inst_name, "bqm", r)
                            row["notes"] = "process_input_failed"
                            row["error"] = str(e)
                ...
                if RUN_NL and nl_writer is not None:
                    ...
                    try:
                        ...
                        print(f"[NL]   [{i + 1}/{len(folders)}] {meta.get('relative_path')} overhead={overhead_s:.3f}s db_cost={db_cost}")
                    except Exception as e:
                        row["error"] = str(e)
                        print(f"[ERR][NL] [{i + 1}/{len(folders)}] {meta.get('relative_path')}: {e}")
                        traceback.print_exc()
                ...
                if RUN_CQM and cqm_writer is not None:
                    ...
                    try:
                        ...
                        # solve_cqm returns D-Wave qpu_access_time converted from microseconds to milliseconds.
                        qpu_time_ms = float(qpu_time) if qpu_time is not None else None
                        qpu_time_s = (qpu_time_ms / 1000.0) if qpu_time_ms is not None else None
                        ...
                        print(f"[CQM]  [{i + 1}/{len(folders)}] {meta.get('relative_path')} overhead={overhead_s:.3f}s qpu_s={qpu_time_s} db_cost={db_cost}")
                    except Exception as e:
                        row["error"] = str(e)
                        print(f"[ERR][CQM] [{i + 1}/{len(folders)}] {meta.get('relative_path')}: {e}")
                        traceback.print_exc()
                ...
                if RUN_BQM and bqm_writer is not None:
                    ...
                    try:
                        ...
                        print(f"[BQM]  [{i + 1}/{len(folders)}] {meta.get('relative_path')} overhead={overhead_s:.3f}s db_cost={db_cost} fallback={bool(use_fallback_join_order)}")
                    except Exception as e:
                        row["error"] = str(e)
                        print(f"[ERR][BQM] [{i + 1}/{len(folders)}] {meta.get('relative_path')}: {e}")
                        traceback.print_exc()
                ...
                if RUN_ITER and iter_writer is not None:
                    ...
                    for emb in CUSTOM_EMBEDDINGS:
                        for iters in ITERATIONS_LIST:
                            row = _make_base_row(run_ts, meta, i, inst_dir, inst_name, "iter", r)
                            row.update({
                            ...
                            try:
                                ...
                                print(
                                    f"\n[ITER] [{i + 1}/{len(folders)}] {meta.get('relative_path')} "
                                ...
                                row["error"] = str(e)
                                print(f"[ERR][ITER] [{i + 1}/{len(folders)}] {meta.get('relative_path')} emb={row['custom_embedding']} iters={iters}: {e}")
                                traceback.print_exc()
    ...
    print(f"[DONE] NL={nl_csv_path} CQM={cqm_csv_path} BQM={bqm_csv_path} ITER={iter_csv_path} LOG={log_path}")
    return nl_csv_path, cqm_csv_path, bqm_csv_path, iter_csv_path, log_path


if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="QDBO join-ordering pipeline over synthetic workloads."
    )
    parser.add_argument(
        "--solvers",
        required=True,
        help="Comma-separated solvers to run: nl,cqm,bqm,iter.",
    )
    parser.add_argument(
        "--iterations",
    ...
    raw_solvers = [
        item.strip().lower() for item in args.solvers.split(",") if item.strip()
    ]
    allowed_solvers = {"nl", "cqm", "bqm", "iter"}
    unknown_solvers = [item for item in raw_solvers if item not in allowed_solvers]
    if unknown_solvers:
        parser.error(f"unknown solvers: {unknown_solvers}")
    RUN_NL = "nl" in raw_solvers
    RUN_CQM = "cqm" in raw_solvers
    RUN_BQM = "bqm" in raw_solvers
    RUN_ITER = "iter" in raw_solvers
