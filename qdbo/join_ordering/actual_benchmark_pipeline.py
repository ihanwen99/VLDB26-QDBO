...
# QDBO join-ordering pipeline over real CEB/JOB workloads.
# Layout:
...
from qdbo.join_ordering.backend.jo_utils import compute_db_cost, get_all_folders_in_target_directory_and_sorted
from qdbo.join_ordering.backend.jo_utils import fetch_query, fetch_latest_result_timeout
...
OUT_DIR = str(RESULTS_ROOT / "join_ordering/actual_cpp_overhead/black-box-solvers")
LOG_DIR = str(RESULTS_ROOT / "join_ordering/actual_cpp_overhead/black-box-solvers/txt")
...
NL_TIME_LIMIT_S = 1
CQM_TIME_LIMIT_S = 5
NL_SAMPLER_LABEL = "QDBO NL Solver (Actual)"
CQM_LABEL = "QDBO CQM (Actual)"
...
# ---- optional PostgreSQL execution harness ----
# OFF by default, so the pipeline stays cost-model-only and needs no database.
# Turn on with --execute or QDBO_EXECUTE=1; requires the workload database
# (imdbload) loaded and pg_hint_plan installed so Leading() hints are honoured.
RUN_EXECUTE = os.environ.get("QDBO_EXECUTE", "").strip().lower() in ("1", "true", "yes")
DEFAULT_EXECUTE_TIMEOUT_MS = 600000
try:
    EXECUTE_TIMEOUT_MS = int(os.environ.get("QDBO_EXECUTE_TIMEOUT_MS",
                                           DEFAULT_EXECUTE_TIMEOUT_MS))
except (TypeError, ValueError):
    print(f"[WARN] ignoring non-integer QDBO_EXECUTE_TIMEOUT_MS="
          f"{os.environ.get('QDBO_EXECUTE_TIMEOUT_MS')!r}; "
          f"using {DEFAULT_EXECUTE_TIMEOUT_MS}")
...
exec_extra = [
    ...
    "planning_ms",
    "exec_ms",
    "baseline_planning_ms",
    "baseline_exec_ms",
    "exec_error",
]


def _note_exec_error(row: Dict[str, Any], msg: str) -> None:
    """Append msg to the row's exec_error, keeping any earlier note."""
    prev = row.get("exec_error") or ""
    row["exec_error"] = f"{prev}; {msg}" if prev else msg


def _attach_execution_metrics(row: Dict[str, Any], query_dir: str) -> None:
    """Measure real PostgreSQL latency for the join order already stored in row.

    Reads the workload .sql file from query_dir and runs it twice through
    EXPLAIN ANALYZE: once unhinted (PostgreSQL's own plan, the baseline) and
    once with the pg_hint_plan Leading() hint built from the solver join order.
    Both paths go through backend.database_connection.get_connection. Fills the
    exec_extra columns in place and never raises, so a database failure records
    an error string instead of aborting the sweep.
    """
    if not RUN_EXECUTE:
        return

    raw_order = row.get("join_order")
    if not raw_order:
        return
    try:
        join_order = json.loads(raw_order) if isinstance(raw_order, str) else list(raw_order)
    except Exception as e:
        row["exec_error"] = f"cannot parse join_order {raw_order!r}: {type(e).__name__}: {e}"
        return

    try:
        query = fetch_query(query_dir)
    except Exception as e:
        row["exec_error"] = f"fetch_query failed: {type(e).__name__}: {e}"
        return

    # Hinted run: Leading(<aliases in solver order>) prepended to the query.
    try:
        planning_ms, exec_ms, _explain, hint = fetch_latest_result_timeout(
            join_order, query, EXECUTE_TIMEOUT_MS
        )
        row["leading_hint"] = hint
        row["planning_ms"] = planning_ms
        row["exec_ms"] = exec_ms
    except Exception as e:
        _note_exec_error(row, f"hinted execution failed: {type(e).__name__}: {e}")

    # Baseline run: identical query with no hint, so PostgreSQL picks the plan.
    try:
        from qdbo.join_ordering.backend.normal_execution import execute_quantum_query
        _explain, base_exec_ms, base_planning_ms = execute_quantum_query(
            query, EXECUTE_TIMEOUT_MS
        )
        row["baseline_exec_ms"] = base_exec_ms
        row["baseline_planning_ms"] = base_planning_ms
    except Exception as e:
        _note_exec_error(row, f"baseline execution failed: {type(e).__name__}: {e}")

    # execute_query/execute_quantum_query return None instead of raising when
    # the connection or the statement fails, so record that explicitly.
    if row.get("exec_ms") is None and row.get("baseline_exec_ms") is None:
        _note_exec_error(
            row,
            "no execution time returned (check the database connection, "
            "QDBO_DB_* settings, and that pg_hint_plan is installed)",
        )
...
def run_actual_cost(
    ...
    # exec_extra is appended only when RUN_EXECUTE is on, so the default CSV
    # schema (and every existing column order) is untouched.
    exec_cols = exec_extra if RUN_EXECUTE else []
    base_fields = base_header + exec_cols
    iter_fields = base_header + iter_extra + exec_cols
            ...
            for benchmark_name, benchmark_root in workload_list:
                ...
                for i, inst_dir in enumerate(folders):
                    ...
                    if need_process_input:
                        try:
                            ...
                            if RUN_NL and nl_writer is not None:
                                ...
                                row["error"] = str(e)
                                nl_writer.writerow({k: row.get(k, None) for k in base_fields})
                                nl_fcsv.flush()
                            if RUN_CQM and cqm_writer is not None:
                                ...
                                row["notes"] = "process_input_failed"
                                row["error"] = str(e)
                                cqm_writer.writerow({k: row.get(k, None) for k in base_fields})
                                cqm_fcsv.flush()
                            if RUN_BQM and bqm_writer is not None:
                                ...
                                row["notes"] = "process_input_failed"
                                row["error"] = str(e)
                                bqm_writer.writerow({k: row.get(k, None) for k in base_fields})
                                bqm_fcsv.flush()
                            continue
                    ...
                    if RUN_NL and nl_writer is not None:
                        ...
                        _attach_execution_metrics(row, inst_dir)
                        nl_writer.writerow({k: row.get(k, None) for k in base_fields})
                        nl_fcsv.flush()
                    ...
                    if RUN_ITER and iter_writer is not None:
                        ...
                        for emb in CUSTOM_EMBEDDINGS:
                            for iters in ITERATIONS_LIST:
                                ...
                                _attach_execution_metrics(row, inst_dir)
                                iter_writer.writerow({k: row.get(k, None) for k in iter_fields})
                                iter_fcsv.flush()
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="QDBO join-ordering pipeline over CEB/JOB workloads."
    )
    parser.add_argument(
        "--solvers",
    ...
    parser.add_argument(
        "--execute",
        action="store_true",
        help="Also execute each join order against PostgreSQL (EXPLAIN ANALYZE, "
             "pg_hint_plan Leading hint) and append latency columns to the CSV. "
             "Off by default; needs the workload database loaded.",
    )
